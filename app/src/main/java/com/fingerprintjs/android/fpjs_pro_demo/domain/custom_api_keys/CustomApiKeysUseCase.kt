...
class CustomApiKeysUseCase @Inject constructor(
    ...
    private suspend fun loadState(): Result<CustomApiKeysState, *> {
        return coroutineBinding {
            val public = appStorage.load(StorageKey.CustomPublicApiKey, String::class.java).bind()
            val secret = appStorage.load(StorageKey.CustomSecretApiKey, String::class.java).bind()
            val region = appStorage.load(StorageKey.CustomApiKeysServerRegion, String::class.java)
                .andThen { runCatching { Configuration.Region.valueOf(it) } }.bind()
            val enabled =
                appStorage.load(StorageKey.CustomApiKeysEnabled, Boolean::class.java).bind()
            CustomApiKeysState(
                public = public,
                secret = secret,
                region = region,
